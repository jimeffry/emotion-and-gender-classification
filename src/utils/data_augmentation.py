...
class ImageGenerator(object):
    ...
    def __init__(self, ground_truth_data, batch_size, image_size,
                train_keys, validation_keys=None,
        ...
        self.dataset_size = len(train_keys)
        self.batch_num_cnt = 0
    ...
    def preprocess_images(self, image_array):
        image_array = (image_array -127.5)*0.0078125
        return image_array
    ...
    def flow(self, mode='train'):
            while True:
                ...
                for key in self.train_keys:
                    ...
                    #print("train img path ",image_path)
                    try:
                        ...
                        if self.ground_truth_transformer != None:
                            ...
                            ground_truth = (
                                self.ground_truth_transformer.assign_boxes(
                                                            ground_truth))
                        elif self.validation_keys is not None:
                    ...
                    if self.batch_num_cnt == np.ceil(self.dataset_size / self.batch_size):
                        self.batch_num_cnt = 0
                        if self.dataset_size % self.batch_size !=0:
                            remainder = self.dataset_size % self.batch_size
                            patch_num = self.dataset_size - remainder
                            cnt_pat =0
                            for pat_key in self.train_keys:
                                image_path = self.path_prefix + pat_key
                                image_array = cv2.imread(image_path)
                                #print("train img path ",image_path)
                                try:
                                    image_array = cv2.resize(image_array,self.image_size)
                                    ...
                                    continue
                                num_image_channels = len(image_array.shape)
                                ground_truth = self.ground_truth_data[pat_key]
                                if image_array is None:
                                    print("pass")
                                    continue
                                if num_image_channels !=3:
                                ...
                                inputs.append(image_array)
                                targets.append(ground_truth)
                                if cnt_pat == patch_num:
                                    break
                    if len(targets) == self.batch_size:
                        self.batch_num_cnt+=1
                        #print("batch ",self.batch_num_cnt)
